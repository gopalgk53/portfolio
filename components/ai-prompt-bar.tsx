...
export function AIPromptBar() {
  ...
  return <div className="w-full max-w-3xl">
    <motion.div layout className={`theme-glass-card overflow-hidden p-5 ${loading?"agent-thinking":""}`} transition={spring}>
      <div className="mb-4 flex items-center justify-between text-xs text-[#64748B]"><span className="flex items-center gap-2"><Terminal className="h-3.5 w-3.5 text-[#2563EB]"/>Ask about my work</span><span className={mode === "fallback" ? "text-[#D97706]" : "text-[#059669]"}>{mode === "fallback" ? "Offline answer" : mode === "cache" ? "Saved answer" : "Online"}</span></div>
      <form onSubmit={submit} className="relative"><Sparkles className="absolute left-3 top-3.5 h-4 w-4 text-[#00A4B4]"/><input aria-label="Ask Gopal AI assistant" value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Ask about experience, systems, or technical decisions…" className="field py-3 pl-10 pr-12 text-sm"/><motion.button aria-label={loading?"Generating response":"Submit question"} whileHover={{scale:1.06}} whileTap={{scale:.94}} transition={spring} disabled={loading} className="absolute right-2 top-2 rounded-lg bg-[#00A4B4] p-2 text-white shadow-[0_0_24px_rgba(0,164,180,.22)]">{loading?<Loader2 className="h-4 w-4 animate-spin"/>:<CornerDownLeft className="h-4 w-4"/>}</motion.button></form>
      ...
      <AnimatePresence>{(stream || loading) && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={spring} className="mt-5 overflow-hidden border-l border-[#2563EB] bg-white/35 px-4 py-3 text-sm leading-6 text-[#334155]"><p className="mb-1 text-xs text-[#64748B]">{loading ? "Writing…" : "Answer"}</p>{stream}<span className="ml-1 inline-block h-3 w-px bg-[#2563EB]" /></motion.div>}</AnimatePresence>
    </motion.div>
  </div>;
}
