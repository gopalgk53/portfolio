...
export function AIPromptBar() {
  ...
  return (
    <div className="w-full max-w-3xl">
      <motion.div layout transition={spring} className="surface-card p-5">
        ...
        <form onSubmit={submit} className="relative">
          ...
          <motion.button
            ...
            transition={spring}
            disabled={loading}
            className="absolute right-2 top-2 rounded-full border border-[var(--border-strong)] bg-[var(--accent)] p-2 text-white"
          >
        ...
        <div className="mt-3 flex flex-wrap gap-x-5 gap-y-2">
          {prompts.map(([label, prompt]) => (
            <button key={label} onClick={() => ask(prompt)} className="border-b border-transparent py-1 text-[11px] text-[var(--muted)] hover:border-[var(--accent)] hover:text-[var(--text)]">
              {label}
            </button>
          ))}
        </div>
