...
export function NeuralBackground() {
  ...
  useEffect(() => {
    ...
    loadThree().then((THREE) => {
      ...
      const pointer={x:0,y:0}, target={x:0,y:0};let scrollTarget=0,last=performance.now(),effectsOff=localStorage.getItem("gopal-reduced-effects")==="true";
      ...
      const render=(now:number)=>{if(effectsOff)return;const dt=Math.min((now-last)/1000,.04);last=now;pointer.x+=(target.x-pointer.x)*.045;pointer.y+=(target.y-pointer.y)*.045;if(!reduced){network.rotation.y+=dt*.035;network.rotation.x+=dt*.012;glyphs.forEach(sprite=>{sprite.position.y-=sprite.userData.speed*dt;if(sprite.position.y < -5)sprite.position.y=5;});}camera.position.x+=(pointer.x*.45-camera.position.x)*.035;camera.position.y+=(-pointer.y*.32-camera.position.y)*.035;camera.position.z+=(12-scrollTarget*.7-camera.position.z)*.035;camera.lookAt(0,0,0);renderer.render(scene,camera);if(!reduced&&!document.hidden)frame=requestAnimationFrame(render);};
      const visibility=()=>{cancelAnimationFrame(frame);if(!document.hidden&&!reduced&&!effectsOff){last=performance.now();frame=requestAnimationFrame(render);}};const effects=(event:Event)=>{effectsOff=(event as CustomEvent).detail.reduced;cancelAnimationFrame(frame);if(!effectsOff){last=performance.now();frame=requestAnimationFrame(render);}};
      addEventListener("pointermove",onPointer,{passive:true});addEventListener("scroll",onScroll,{passive:true});addEventListener("resize",onResize,{passive:true});addEventListener("gopal-effects",effects);document.addEventListener("visibilitychange",visibility);onScroll();if(!effectsOff)frame=requestAnimationFrame(render);
      cleanup=()=>{cancelAnimationFrame(frame);removeEventListener("pointermove",onPointer);removeEventListener("scroll",onScroll);removeEventListener("resize",onResize);removeEventListener("gopal-effects",effects);document.removeEventListener("visibilitychange",visibility);particleGeometry.dispose();particles.material.dispose();lineGeometry.dispose();lines.material.dispose();glyphTexture.dispose();glyphMaterial.dispose();renderer.dispose();};
