...
export function PromptPlayground(){
  ...
  return <div>
    <div className="mb-5 flex items-center justify-between gap-4"><p className="text-xs text-slate-500">Status: {status}</p><motion.button whileHover={{scale:1.03}} whileTap={{scale:.96}} transition={spring} onClick={reset} className="flex items-center gap-2 rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-600"><RotateCcw className="h-3 w-3"/>Reset</motion.button></div>
    <div className="grid gap-5 xl:grid-cols-[1.05fr_.9fr_1.25fr]">
      <motion.article animate={{opacity:focus===0||focus===1?1:.55}} transition={spring} className={`glass-card flex min-h-[500px] flex-col p-5 ${status==="error"?"!border-red-500/70":""}`} onFocus={()=>setFocus(1)} onBlur={()=>setFocus(0)}>
        <header className="flex justify-between text-xs"><span className="font-medium text-slate-700">Write a prompt</span><span className={status==="error"?"text-red-600":"text-slate-400"}>{status==="error"?"Add a prompt":prompt?"Ready":"Empty"}</span></header>
        <div className="relative mt-5"><span className="absolute left-3 top-3 font-mono text-sm text-emerald-400">›</span><textarea aria-label="Prompt playground input" value={prompt} onChange={e=>{setPrompt(e.target.value);if(status==="error")setStatus("ready")}} rows={8} placeholder={status==="error"?"[SYSTEM ERROR: Null input detected. Please provide a semantic prompt sequence.]":"Enter a prompt to inspect model behavior..."} className="w-full resize-none rounded-xl border border-cyan-400/15 bg-black/70 py-3 pl-8 pr-3 font-mono text-xs leading-6 text-slate-200 outline-none focus:border-cyan-400/60 focus:shadow-[0_0_24px_rgba(0,242,254,.08)]"/></div>
        ...
        <motion.button onClick={execute} disabled={status==="computing"||status==="streaming"} whileHover={{y:-2}} whileTap={{scale:.98}} transition={spring} className="mt-auto self-end rounded-lg bg-blue-600 px-5 py-3 text-sm font-semibold text-white shadow-sm disabled:opacity-40">{status==="computing"||status==="streaming"?"Generating…":"Run prompt"}</motion.button>
      </motion.article>
      ...
      <motion.article animate={{opacity:focus===0||focus===2?1:.55}} transition={spring} className="glass-card min-h-[500px] p-5" onPointerDown={()=>setFocus(2)}>
        <header className="flex justify-between text-xs"><span className="font-medium text-slate-700">Compare approaches</span><span className="text-slate-400">Basic ↔ structured</span></header>
      ...
      <motion.article animate={{opacity:focus===0||focus===3?1:.55}} transition={spring} className="glass-card flex min-h-[500px] flex-col p-5" onPointerDown={()=>setFocus(3)}>
        <header className="flex justify-between text-xs"><span className="font-medium text-slate-700">Example output</span><span className="text-emerald-600">{status==="idle"||status==="ready"||status==="error"?"Waiting":status}</span></header>
